...
if redis:get(blcon..'bot:name') then
bot_name = redis:get(blcon..'bot:name') 
else
bot_name = 'فير'
end
...
if r=="المشتركين" and is_sudo(msg) and msg.to.type == 'pv'  then
  local users = '💢¦ عدد المشتركين  `'..redis:scard(blcon..'users')..'` *مشترك في البوت *🍃'
return users
end

if r=="/start" and msg.to.type == 'pv'  then
 redis:sadd(blcon..'users',msg.from.id)
...
redis:setex(blcon..'namebot:witting'..msg.from.id,300,true)
...
if not we_sudo(msg) then return "☔️هذا الاوامر للمطور الاساسي فقط 🌑" end
redis:setex(blcon..'addrd_all:'..msg.from.id,300 , true)
redis:del(blcon..'allreplay:'..msg.from.id)
return "📭¦ _حسننا الان ارسل كلمة الرد العام _🍃\n"
end
------------------------------------------------------
if msg.text then
if redis:get(blcon..'namebot:witting'..msg.from.id) then --- استقبال اسم البوت
redis:del(blcon..'namebot:witting'..msg.from.id)
redis:set(blcon..'bot:name',msg.text)
reload_plugins( )
return "📭¦ تم تغير اسم البوت  ✋🏿\n🗯¦ الان اسمه `"..check_markdown(r).."` 🍃"
end

if redis:get(blcon..'addrd_all:'..msg.from.id) then -- استقبال الرد لكل المجموعات
if not redis:get(blcon..'allreplay:'..msg.from.id) then-- استقبال كلمه الرد لكل المجموعات
redis:setex(blcon..'allreplay:'..msg.from.id, 300 , msg.text )
return "💢¦ _شكرأ لك 😻_\n💢¦ _الان ارسل جواب الرد _✔️" 
end
if redis:get(blcon..'allreplay:'..msg.from.id) then -- استقبال جواب الرد لكل المجموعات
redis:hset(blcon..'replay:all', redis:get(blcon.."allreplay:"..msg.from.id), msg.text)
redis:del(blcon..'addrd_all:'..msg.from.id)
return '('..check_markdown(redis:get(blcon..'allreplay:'..msg.from.id))..')\n  ✔️ تم اضافت الرد لكل المجموعات 🚀 '
end
end
-------------------------------------------------------------
if redis:get(blcon..'addrd:'..msg.from.id) then -- استقبال الرد للمجموعه فقط
if not redis:get(blcon..'replay1'..msg.from.id) then  -- كلمه الرد
redis:setex(blcon..'replay1'..msg.from.id,300,msg.text)
return "💢¦ _شكرأ لك 😻_\n💢¦ _الان ارسل جواب الرد _✔️" 
end
if redis:get(blcon..'replay1'..msg.from.id) then -- جواب الرد
redis:hset(blcon..'replay:'..msg.to.id, redis:get(blcon.."replay1"..msg.from.id), msg.text)
redis:del(blcon..'addrd:'..msg.from.id)
return '('..check_markdown(redis:get(blcon..'replay1'..msg.from.id))..')\n  ✔️ تم اضافت الرد 🚀 '
end
end

--------------------------------------
if redis:get(blcon..'delrd:'..msg.from.id) then
redis:del(blcon..'delrd:'..msg.from.id)

if not redis:hget(blcon..'replay:'..msg.to.id,msg.text) then
return '🗯هذا الرد ليش مضاف في قائمه الردود 💯'
else
redis:hdel(blcon..'replay:'..msg.to.id,msg.text)
return '('..check_markdown(msg.text)..')\n  ✔️ تم مسح الرد 🚀 '
end
end
-------------------------------------
if redis:get(blcon..'delrdall:'..msg.from.id) then
redis:del(blcon..'delrdall:'..msg.from.id)
if not redis:hget(blcon..'replay:all',msg.text) then
return '🗯هذا الرد ليش مضاف في قائمه الردود 💯'
else
redis:hdel(blcon..'replay:all',msg.text)
return '('..check_markdown(msg.text)..')\n  ✔️ تم مسح الرد 🚀 '
end
end
...
local names = redis:hkeys(blcon..'replay:'..msg.to.id)
for i=1, #names do
redis:hdel(blcon..'replay:'..msg.to.id,names[i])
end
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end
local names = redis:hkeys(blcon..'replay:all')
for i=1, #names do
redis:hdel(blcon..'replay:all',names[i])
end
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end
redis:set(blcon..'delrdall:'..msg.from.id,true)
return "📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل الرد لمسحها من  المجموعات 🍃"
end

if r== 'مسح رد' then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end
redis:set(blcon..'delrd:'..msg.from.id,true)
...
return "👨🏾‍🔧¦ لا يوجد ردود مضافه للمجموعـةة ✋🏿"
end
end
...
if not redis:get(blcon..'group:add'..msg.to.id) then return end

--------------------[Test Bot]----------------------------
if r=="اضف رد" then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 💯" end
redis:setex(blcon..'addrd:'..msg.from.id,300 , true)
redis:del(blcon.."replay1"..msg.from.id)
...
if redis:get(blcon..'replay'..msg.to.id) then
...
if string.match(r2, 'عمر')  or  string.match(r2, 'عموش') or  string.match(r2, 'عموري') or  string.match(r2, 'عمور')  or string.match(r2, 'عمروش') then
return true
else
return false
end
...
----------------------------------------------
if redis:hget(blcon..'replay:all', r) then
return check_markdown(redis:hget(blcon..'replay:all', r))
elseif redis:hget(blcon..'replay:'..msg.to.id, r) then
return check_markdown(redis:hget(blcon..'replay:'..msg.to.id, r))
...
if string.len(r2) > 60 then return "💢¦ ما اكدر اكول اكثر من 60 حرف 🙌🏾" end
if sudoname(r2) then return "📌 ما اكدر احجي عليه مستحيل 🕵🏻" end
if linkmsg(r2) then
...
if string.len(r2) > 60 then return "💢¦ ما اكدر اكله اكثر من 60 حرف 🙌🏾" end
if sudoname(r2) then return "📌 ما اكدر احجي عليه مستحيل 🕵🏻" end
if linkmsg(r2) then
return "انته لوتي عود ؟ هو اني نغل ههه يريدني انشر رابط او معرف 😪 " 
end
